...
from simple_xiaozhi.constants.constants import AbortReason, DeviceState
from simple_xiaozhi.plugins.base import Plugin
...
class UIPlugin(Plugin):
    ...
    def _create_display(self):
        ...
        """
        if self.mode == "gui":
            from simple_xiaozhi.display.gui_display import GuiDisplay

            self._is_gui = True
            return GuiDisplay()
        else:
            from simple_xiaozhi.display.cli_display import CliDisplay
    ...
    async def _press(self):
        ...
        手动模式：按下开始录音.
        """
        await self.app.start_auto_conversation(force_realtime=True)
